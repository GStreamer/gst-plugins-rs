...
impl Session {
    ...
    fn handle_timeouts(&mut self, now: Instant) -> Vec<u32> {
        ...
        while let Some((lru_ssrc, lru_source)) = self.remote_senders.peek_lru() {
            if now - lru_source.last_activity() < td {
                break;
            }
            timed_out_ssrcs.push(*lru_ssrc);
            self.remote_senders.pop_lru();
        }
        while let Some((lru_ssrc, lru_source)) = self.remote_receivers.peek_lru() {
            if now - lru_source.last_activity() < td {
                break;
            }
            timed_out_ssrcs.push(*lru_ssrc);
